...
function get_user_by_username($username){
    $data_user =  db_fetch_row("SELECT * FROM `tbl_users` WHERE `username` = '{$username}' ");
    if( !empty($data_user) )
        return $data_user;
}
...
function get_password_by_username($username){
    $password_user =  db_fetch_row("SELECT `password` FROM `tbl_users` WHERE `username` = '{$username}' ");

    return $password_user;
}

function update_password($data , $username){
    return db_update('tbl_users',$data," `username` = '{$username}' ");
}